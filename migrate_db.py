...
def add_skolni_rok_column():
    ...
    with app.app_context():
        ...
        if 'skolni_rok' not in columns:
            ...
            db.session.execute(text('ALTER TABLE student_scores ADD COLUMN skolni_rok INTEGER'))
            
            # Aktualizace existujících záznamů
            db.session.execute(text(f'UPDATE student_scores SET skolni_rok = {skolni_rok}'))
            db.session.commit()
